...
class graph_w_names:
    def __init__(self, names, edges):
        ...
        if self.edges is None or len(self.edges) == 0:
            # Handle the case of no edges by creating an empty graph
            self.graph = csr_matrix((len(names), len(names)))
            ...
            # Check if edges_indices is not empty before unpacking
            if edges_indices:
                rows, cols = zip(*edges_indices)
                data = np.ones(len(edges_indices), dtype=int)
                self.graph = csr_matrix((data, (rows, cols)), shape=(len(names), len(names)))
            else:
                # Handle empty edges_indices
                self.graph = csr_matrix((len(names), len(names)))
