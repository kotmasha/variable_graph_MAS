...
class netwk():
    ...
    def __init__(self,netID,graphWithNames,env,leaders,pnpParameters,stateWname=None):
        ...
        self.goalVisual=self.visualization.plot(self.leaders['Joe']['Target'][0],self.leaders['Joe']['Target'][1],'rx')
    ...
    def populate(self,mode=0):
        if mode==0:
            ...
            for name,pos in self.stateWname:
                self.agents[name]=baseAgent(name, self.env, self,self.task.taskList[name], np.array(pos).reshape((2,1)))
        else:
    ...
    # go over dfs ordering of the vertices
        ...
        for item,predName in zip(dfsOrder,dfsPredecessors):
            ...
            self.agents[item]=baseAgent(item, self.env, self,self.task.taskList[item], pt)
        return self.agents
